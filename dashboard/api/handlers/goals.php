<?php
/**
 * DGD Dashboard - Goals / OKR Handlers
 *
 * Endpoints:
 ...
 *
 * Uses JOIN to fetch goals + key_results in one query (fixes N+1).
 */
function handle_list_goals(): void
{
    ...
    $wc = build_where_clause([
        ...
        'quarter' => ['column' => 'quarter', 'type' => 'int'],
        'year'    => ['column' => 'year',    'type' => 'int'],
        'status'  => ['column' => 'status',  'type' => 'string'],
        'owner'   => ['column' => 'owner',   'type' => 'string'],
    ], 'g');

    $whereClause = $wc['clause'];
    $params = $wc['params'];

    $stmt = $db->prepare("
        SELECT g.*,
               kr.id as kr_id, kr.title as kr_title, kr.metric as kr_metric,
               kr.current_value as kr_current_value, kr.target_value as kr_target_value,
               kr.unit as kr_unit, kr.kpi_id as kr_kpi_id, kr.status as kr_status,
               kr.created_at as kr_created_at, kr.updated_at as kr_updated_at
        FROM goals g
    ...
    $goals = group_children($rows, 'kr_', 'key_results');
...
function handle_update_goal(string $id): void
{
    ...
    require_exists('goals', $id, 'Goal');

    do_partial_update('goals', $id, $body,
        ['title', 'description', 'type', 'owner', 'quarter', 'year', 'status', 'progress', 'parent_goal_id'],
        ['quarter' => 'int', 'year' => 'int', 'progress' => 'float']
    );
...
function handle_create_key_result(string $goalId): void
{
    ...
    require_exists('goals', $goalId, 'Goal');
